        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
            ...
            <textarea
              ...
              className="w-full h-56 mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-5 text-lg resize-none outline-none focus:border-blue-500"
            />
            ...
            <div className="flex items-center justify-between mt-5">
              ...
              <button
                onClick={analyzeText}
                disabled={loading}
                className="flex items-center gap-3 px-8 py-4 bg-blue-500 text-white rounded-2xl font-semibold shadow-lg hover:bg-blue-600 transition disabled:opacity-50"
              >
                <Search size={20} />
              ...
              <div className="space-y-3">
                {examples.map((item, index) => (
                  <button
                    ...
                    className="w-full text-left bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 hover:bg-blue-50 transition"
                  >
                    {item}
                  </button>
                ))}
              ...
              <div>
                  ...
                  <div className="w-full h-4 bg-slate-200 rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full transition-all duration-700 ${
                        result.prediction === "Hate Speech" ? "bg-red-500" : "bg-green-500"
                      }`}
                ...
                <div className="grid grid-cols-2 gap-5 mt-8">
                  <div className="bg-slate-50 rounded-2xl p-5">
                    ...
                    <h3 className="font-bold text-xl mt-2">{result.prediction}</h3>
                  </div>
                  <div className="bg-slate-50 rounded-2xl p-5">
                    <p className="text-slate-500">Status</p>
                    <h3
                      className={`font-bold text-xl mt-2 ${
                        result.prediction === "Hate Speech" ? "text-red-600" : "text-green-600"
                      }`}
                    >
                      {result.prediction === "Hate Speech" ? "Unsafe" : "Safe"}
                    </h3>
                  </div>
                </div>
                ...
                <div className="mt-8 rounded-2xl border border-blue-200 bg-blue-50 p-6">
                  <h3 className="font-bold text-blue-700">Result</h3>
                ...
                <div className="mt-8 bg-slate-50 rounded-2xl p-6">
                  <h3 className="font-semibold">Ujaran</h3>
                  <p className="mt-3 text-slate-600 leading-7">{result.text}</p>
                </div>
              </div>
            )}
          </div>
